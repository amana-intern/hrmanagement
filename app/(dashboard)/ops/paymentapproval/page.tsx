...
export default function PaymentRequestPage() {
  ...
  const handleAction = async (id: string, action: string, catatan?: string) => {
    ...
    if (ok) setMessage({ ok: true, text: action === 'reject' ? 'Request successfully rejected.' : 'Request successfully processed.' });
  };
  ...
  const renderAction = (r: PayReq) => (
    <ApprovalActions
      disabled={r.status !== PAYMENT_STATUS.PENDING_OPS || processingId === r.id}
