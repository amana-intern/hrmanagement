...
export default function PaymentPage() {
  ...
  // Styling umum input disesuaikan dengan tema AMANA
  const inputClassName = "w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:border-amana-blue focus:ring-1 focus:ring-amana-blue outline-none text-sm font-normal transition-all text-slate-700 shadow-sm";
  const labelClassName = "block text-sm font-bold text-amana-blue mb-1.5";
  ...
  const UploadBox = ({ label, fileKey }: { label?: string; fileKey: string }) => (
    <div className="relative w-full border border-amana-blue rounded-xl p-6 flex flex-col items-center justify-center bg-white hover:bg-amana-blue/5 transition cursor-pointer mt-1 shadow-sm">
      <input 
        type="file" 
      ...
      <span className="text-[11px] font-bold text-amana-blue uppercase tracking-wider text-center truncate max-w-full px-2">
  ...
  return (
    <div className="flex w-full h-screen overflow-hidden bg-amana-white font-sans">
      ...
      {/* Main Content Area: overflow diatur dinamis berdasarkan step */}
      <main className={`flex-1 p-6 text-slate-800 flex flex-col items-center justify-center ${step === 2 ? 'overflow-y-auto py-8 justify-start' : 'overflow-hidden'}`}>
        
        {/* Wrapper pembatas agar judul dan card berada di tengah secara simetris */}
        <div className="w-full max-w-3xl flex flex-col">
          
          <h1 className="text-2xl md:text-3xl font-bold mb-4 text-amana-blue">
            Payment Submission
          </h1>

          <div className="bg-white p-6 md:p-7 rounded-2xl border border-slate-200 shadow-sm w-full flex flex-col gap-4">
            
            {/* ======================= STEP 1 ======================= */}
            {step === 1 && (
              <>
                <h2 className="text-lg font-bold text-amana-blue mb-1 border-b pb-2">Step 1: General Details</h2>
                ...
                <div className="flex justify-end mt-2 pt-3 border-t border-slate-100">
                  <button 
                    ...
                    className={`px-7 py-2.5 rounded-xl font-semibold text-sm transition-all
                      ${isStep1Complete 
                        ? "bg-amana-blue hover:opacity-90 text-white cursor-pointer shadow-sm" 
            ...
            {step === 2 && (
              <>
                <div className="flex items-center gap-3 mb-1 border-b pb-3">
                  <button 
                    onClick={() => setStep(1)} 
                    className="text-slate-400 hover:text-amana-blue transition flex items-center justify-center p-1"
                  ...
                  <h2 className="text-lg font-bold text-amana-blue">Step 2: Payment Details</h2>
                </div>

                <div>
                  <label className="block text-xl font-normal text-amana-blue mb-2">To whom is this payment for</label>
                  <select 
                    value={paymentFor} 
                    onChange={(e) => {
                ...
                {paymentFor === "Vendor" && (
                  <div className="flex flex-col gap-4 animate-fade-in mt-2">
                    <div>
                      ...
                      <input type="text" placeholder="Enter vendor name" className={inputClassName} />
                    </div>
                    ...
                    <div className="flex justify-end mt-2 pt-3">
                      <button type="button" className="px-7 py-2.5 bg-amana-blue text-white rounded-xl hover:opacity-90 font-semibold text-sm transition shadow-sm">
                        Submit Payment
                      </button>
                    </div>
                  </div>
                ...
                {paymentFor === "Individual(s)" && (
                  <div className="flex flex-col gap-4 animate-fade-in mt-2">
                    <div>
                      ...
                      <input type="text" placeholder="Activity name" className={inputClassName} />
                    </div>
                    
                    <div>
                      <label className={labelClassName}>Name of the Honor Receiver</label>
                      <input type="text" placeholder="Receiver name" className={inputClassName} />
                    </div>
                    ...
                    <div>
                      ...
                      {individualRole === "Other" && (
                        <input type="text" placeholder="Please specify role..." className={`${inputClassName} mt-2`} />
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input type="text" placeholder="Account Name" className={inputClassName} />
                      </div>
                      <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input type="text" placeholder="Component" className={inputClassName} />
                      </div>
                      <div>
                    ...
                    <div>
                      <label className={labelClassName}>
                        Copy of Individual KTP <span className="font-normal text-slate-400 ml-1">(For tax purposes)</span>
                      </label>
                    ...
                    <div>
                      <label className={labelClassName}>
                        Attach Invoice <span className="font-normal text-slate-400 ml-1">(Optional)</span>
                      </label>
                  ...
                  <div className="flex flex-col gap-4 animate-fade-in mt-2">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input type="text" placeholder="Event Name" className={inputClassName} />
                      </div>
                      <div>
                    ...
                    <div className="mt-1 bg-amana-white rounded-xl p-4 border border-slate-200">
                      <h3 className="text-base font-bold text-amana-blue mb-2">Upload file with participant details</h3>
                      <p className="text-xs text-slate-600 mb-3 font-medium">Please ensure the document includes:</p>
                      <div className="grid grid-cols-2 gap-y-1 text-xs text-slate-700 font-bold mb-4">
                        <p>1. Full Name</p>
                        <p>2. Phone Number</p>
                        <p>3. Organization</p>
