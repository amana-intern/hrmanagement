...
export async function PATCH(
  ...
  try {
    ...
    // Catat Career History: satu entri per aksi edit, menggabungkan semua field yang benar-benar berubah.
    ...
    if (department !== undefined && (data.department ?? null) !== oldDept) {
      perubahan.push({
        ...
        from: oldDept ? (DEPARTMENT_LABELS[oldDept] ?? oldDept) : '-',
        to: data.department ? (DEPARTMENT_LABELS[data.department] ?? data.department) : '-',
      });
    }
    if (namaGrade !== undefined && String(namaGrade).trim() && newGradeName !== oldGrade) {
      perubahan.push({ field: 'Grade Transfer', from: oldGrade ?? '-', to: newGradeName ?? '-' });
    }
    if (idRole && newRoleName !== oldRole) {
      perubahan.push({ field: 'Role Change', from: oldRole ?? '-', to: newRoleName ?? '-' });
    }
